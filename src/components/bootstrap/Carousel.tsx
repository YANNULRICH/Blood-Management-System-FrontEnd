import React, {forwardRef, ReactNode, SetStateAction, useRef, useState} from 'react';
...
import "../../components/bootstrap/YourStylesheet.scss"
...
interface ICarouselProps {
	...
	items?: {
		...
		type?: 'IMAGE' | 'VIDEO';
		isDetail?: boolean
		height?: string;  // Ajoutez ceci
		width?: string;   // Ajoutez ceci
		marginRight?: string;  // Ajoutez ceci
		objectFit?: string;  // Ajoutez ceci
	}[];
	children?: ReactNode;
...
const Carousel = forwardRef<HTMLDivElement, ICarouselProps>(
	(
		...
		const currentImage = useRef<string | undefined>("")
			...
			if (items) {
				...
				return (
					<>
						{_item.type === 'IMAGE' && (
							<>
								<div className="cursor-pointer" onClick={() => {currentImage.current = _item.src; setShowImage(true)}}>
									<img style={{ cursor: 'pointer' }} src={_item.src} alt={_item.altText}
										 className= {classNames('d-block w-100',{'publicationImage': !_item.isDetail, 'publicationDetailImage': _item.isDetail})}
									/>
								</div>
							...
							<>
								{showVid ? (
									<video controls autoPlay={true}
										   className= {classNames('d-block w-100',{'publicationImage': !_item.isDetail, 'publicationDetailImage': _item.isDetail})}
									>
										<source src={_item.src} type="video/mp4" />
										Votre navigateur ne prend pas en charge les vidéos HTML5.
									</video>
								) : (
									<div className='position-relative'>
										<img src={ _item.tomail?.thumbnailPath || _item.tomail} alt={_item.altText}
											 className= {classNames('d-block w-100',{'publicationImage': !_item.isDetail, 'publicationDetailImage': _item.isDetail})}
										/>
										{/*<Button className='position-absolute top-25 start-25' onClick={() => setShowVid(true)}>*/}
						...
						<DialogComponent show={showImage} size='xl' title={""} onClose={() => setShowImage(false)} >
							<img src={currentImage.current} alt={"img"}/>
						</DialogComponent>
					</>
				);
			}
		...
		return (
			<CarouselContainer
				...
				{isControl && items?.length as number > 1 && (
					<>
						<CarouselControl
							id={id}
							direction='prev'
